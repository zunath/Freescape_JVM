...
public class BladePowerAttack implements IPerk {
    @Override
    ...
    private void ApplyFeatChanges(NWObject oPC, NWObject oItem)
    {
        ...
        int perkLevel = PerkSystem.GetPCPerkLevel(oPC, PerkID.BladePowerAttack);

        if(equipped.equals(oItem) || itemGO.getCustomItemType() != CustomItemType.Blade)
        {
            NWNX_Creature.RemoveFeat(oPC, Feat.POWER_ATTACK);
            NWNX_Creature.RemoveFeat(oPC, Feat.IMPROVED_POWER_ATTACK);
            return;
