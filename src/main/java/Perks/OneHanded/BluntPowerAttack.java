...
public class BluntPowerAttack implements IPerk {
    @Override
    ...
    public void OnRemoved(NWObject oPC) {
        NWNX_Creature.RemoveFeat(oPC, Feat.POWER_ATTACK);
        NWNX_Creature.RemoveFeat(oPC, Feat.IMPROVED_POWER_ATTACK);
    }
    ...
    private void ApplyFeatChanges(NWObject oPC, NWObject oItem)
    {
        ...
        int perkLevel = PerkSystem.GetPCPerkLevel(oPC, PerkID.BluntPowerAttack);

        if(equipped.equals(oItem) || itemGO.getCustomItemType() != CustomItemType.Blunt)
        {
            NWNX_Creature.RemoveFeat(oPC, Feat.POWER_ATTACK);
            NWNX_Creature.RemoveFeat(oPC, Feat.IMPROVED_POWER_ATTACK);
            return;
        ...
        NWNX_Creature.AddFeat(oPC, Feat.POWER_ATTACK);

        if(perkLevel >= 2)
        {
            NWNX_Creature.AddFeat(oPC, Feat.IMPROVED_POWER_ATTACK);
        }
