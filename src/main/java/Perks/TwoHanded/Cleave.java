...
public class Cleave implements IPerk {
    @Override
    ...
    public void OnRemoved(NWObject oPC) {
        NWNX_Creature.RemoveFeat(oPC, Feat.CLEAVE);
    }
    ...
    private void ApplyFeatChanges(NWObject oPC, NWObject oItem)
    {
        ...
        if(equipped.equals(oItem) || itemGO.getCustomItemType() != CustomItemType.HeavyBlade)
        {
            NWNX_Creature.RemoveFeat(oPC, Feat.CLEAVE);
            return;
        }

        NWNX_Creature.AddFeat(oPC, Feat.CLEAVE);
    }
